...
export const JudgeRolePicker: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 p-6 md:p-12 flex flex-col items-center justify-between font-sans selection:bg-sky-500 selection:text-white">
      {/* Background Glow */}
      <div className="fixed inset-0 bg-[linear-gradient(to_right,rgba(255,255,255,0.03)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-[size:3rem_3rem] pointer-events-none" />
      ...
      <div className="w-full max-w-6xl flex items-center justify-between z-10">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="font-extrabold text-sm tracking-tight text-white">SALAY</span>
      ...
      <div className="w-full max-w-5xl my-12 space-y-10 z-10 text-center">
        ...
        <div className="space-y-4 max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-neutral-900 border border-neutral-800 rounded-full text-xs font-mono text-neutral-300 shadow-xl">
          ...
          <p className="text-sm text-neutral-400 max-w-xl mx-auto leading-relaxed">
            Test SALAY as an Auditor, Official, Admin, or Citizen. All demo actions execute in an <strong className="text-neutral-200">Ephemeral Session Sandbox</strong> and reset on exit.
          </p>
        </div>

        {/* 3-Step Demo Guide Banner */}
        <div className="p-4 bg-neutral-900/90 border border-neutral-800 rounded-2xl text-left grid grid-cols-1 md:grid-cols-3 gap-4 text-xs backdrop-blur-xl">
          <div className="flex items-start space-x-3">
            <div className="w-6 h-6 rounded-full bg-sky-500/20 text-sky-400 flex items-center justify-center font-bold text-xs shrink-0">1</div>
            <div>
              <strong className="text-white block font-semibold mb-0.5">Choose a Persona</strong>
              <span className="text-neutral-400">Click any card below to launch directly into the dashboard as that role.</span>
            </div>
          </div>
          <div className="flex items-start space-x-3">
            <div className="w-6 h-6 rounded-full bg-sky-500/20 text-sky-400 flex items-center justify-center font-bold text-xs shrink-0">2</div>
            <div>
              <strong className="text-white block font-semibold mb-0.5">Test All Features</strong>
              <span className="text-neutral-400">Inspect PDF contract proofs, query Cortex AI, or run CoCo CLI commands.</span>
            </div>
          </div>
          <div className="flex items-start space-x-3">
            ...
            <div>
              <strong className="text-white block font-semibold mb-0.5">Take Guided Tour</strong>
              <span className="text-neutral-400">Click "Take Demo Tour" in the top bar anytime for step-by-step highlights!</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
          {personas.map((p) => {
            ...
            return (
              <div
                key={p.role}
                className={`p-6 bg-neutral-900/80 border ${p.color} rounded-2xl shadow-xl flex flex-col justify-between space-y-6 hover:scale-[1.01] transition-all backdrop-blur-xl group`}
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-mono font-bold uppercase px-2.5 py-1 rounded-full bg-neutral-950 border border-neutral-800 text-neutral-300">
                      {p.badge}
                    </span>
                  </div>

                  <div>
                    <h2 className="text-lg font-bold text-white group-hover:text-sky-400 transition-colors">{p.title}</h2>
                ...
                <button
                  ...
                  className={`w-full py-3 px-4 text-xs font-bold rounded-xl text-white shadow-lg transition-all active:scale-95 flex items-center justify-center space-x-2 ${p.btnBg}`}
                >
      ...
      {/* Footer */}
      <div className="w-full max-w-5xl text-center text-xs text-neutral-500 pt-6 border-t border-neutral-900 font-mono z-10 flex flex-col sm:flex-row items-center justify-between gap-2">
